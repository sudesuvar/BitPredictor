...
class TemporalFusionTransformer(tf.keras.Model):
    def __init__(self, units=64, num_heads=4, num_layers=2, head_size=64, dropout=0.1):
        super(TemporalFusionTransformer, self).__init__()
        
        # TFT'in temel bileşenleri
        self.encoder = layers.MultiHeadAttention(num_heads=num_heads, key_dim=units, dropout=dropout)
        self.dense1 = layers.Dense(units, activation='relu')
        self.dense2 = layers.Dense(1)
        
        # TFT için özel ek bileşenler (örneğin, static ve dynamic encoding)
        self.dropout = layers.Dropout(dropout)

    def call(self, inputs):
        # Encoder kısmı (multi-head attention)
        x = self.encoder(inputs, inputs)
        
        # TFT modelindeki ek katmanlar
        x = self.dense1(x)
        x = self.dropout(x)
