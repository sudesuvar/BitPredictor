...
class Informer(tf.keras.Model):
    def __init__(self, units=64, num_heads=4, dropout_rate=0.1):
        super(Informer, self).__init__()
        self.encoder_layer = layers.MultiHeadAttention(num_heads=num_heads, key_dim=units)
        self.dense_projection = layers.Dense(units, activation='relu')
        self.dropout = layers.Dropout(dropout_rate)
        self.final_dense = layers.Dense(1)

    def call(self, inputs):
        # Attention mekanizması
        attn_output = self.encoder_layer(inputs, inputs)
        # Projeksiyon katmanı
        proj_output = self.dense_projection(attn_output)
        # Dropout uygulanıyor
        proj_output = self.dropout(proj_output)
        # Çıkış için son dense katmanı
        return self.final_dense(proj_output[:, -1, :])
...
# Hata yüzdesi hesaplama (mutlak değer üzerinden)
errors = np.abs((y_test - y_pred.flatten()) / y_test) * 100

# Daha düzgün grafik için seaborn ile görselleştirme
plt.figure(figsize=(14, 7))
sns.lineplot(x=dates, y=errors, color='purple', label='Hata Payı (%)', linewidth=2)
